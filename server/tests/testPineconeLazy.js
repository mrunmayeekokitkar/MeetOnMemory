...
assert.ok(embeddingUtils.initVectorStore, "initVectorStore function should be exported");
assert.ok(embeddingUtils.searchVectorStore, "searchVectorStore function should be exported");
assert.ok(embeddingUtils.indexMeeting, "indexMeeting function should be exported");

console.log("✅ embeddingUtils imported cleanly without eager Pinecone initialization.");
...
if (!process.env.PINECONE_API_KEY) {
  try {
    ...
    assert.fail("initVectorStore should have thrown an error when PINECONE_API_KEY is missing");
  } catch (err) {
    assert.ok(
      err.message.includes("PINECONE_API_KEY") || err.message.includes("INDEX_NAME"),
