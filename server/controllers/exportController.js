...
export const exportMeeting = async (req, res) => {
  try {
    ...
    if (format === "pdf") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_mom.pdf"`,
      );
      doc.pipe(res);
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_mom.docx"`,
      );
      res.send(buffer);
    } else if (format === "md") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_mom.md"`,
      );
      res.send(md);
    } else {
      return res
